...
class NFSPAgent(Agent):
    ...
    def __init__(self,
                    ...
                    reservoir_buffer_capacity:int=20000,
                    anticipatory_param:float=0.1,
                    batch_size:int=256,
                    train_every:int=1,
                    rl_learning_rate:float=0.1,
                    sl_learning_rate:float=0.005,
                    min_buffer_size_to_learn:int=100,
                    q_replay_memory_size:int=20000,
                    q_replay_memory_init_size:int=100,
                    q_update_target_estimator_every:int=1000,
                    q_discount_factor:float=0.99,
                    q_epsilon_start:float=0.06,
                    q_epsilon_end:float=0,
                    q_epsilon_decay_steps:int=int(1e6),
                    q_batch_size:int=32,
                    q_train_every:int=1,
        ...
        Args:
            ...
            hidden_layers_sizes (list): The hidden layers sizes for the layers of the average policy.
            ...
            q_update_target_estimator_every (int): The frequency of updating target network for inner DQN agent.
    ...
    def _build_model(self)->None:
        ...
        policy_network = AveragePolicyNetwork(
            self._num_actions, self._state_shape, self._layer_sizes)
    ...
    def eval_step(self, state:dict)->tuple[int,dict]:
        ...
        if self.evaluate_with == 'best_response':
            ...
            action = np.random.choice(len(probs), p=probs)
            info = {}
            info['probs'] = {state['raw_legal_actions'][i]: float(probs[list(
    ...
    def sample_episode_policy(self)->None:
    ...
    def _add_transition(self, state:np.array, probs:np.array)->None:
    ...
    def train_sl(self)->float:
        ...
        # (batch, state_size)
        info_states = torch.from_numpy(
            np.array(info_states)).float().to(self.device)
    ...
    @classmethod
    def from_checkpoint(cls, checkpoint:dict)->None:
        '''
        ...
        agent.policy_network = AveragePolicyNetwork.from_checkpoint(
            checkpoint['policy_network'])
        agent._reservoir_buffer = ReservoirBuffer.from_checkpoint(
            checkpoint['reservoir_buffer'])
        ...
        agent.policy_network_optimizer = torch.optim.Adam(
            agent.policy_network.parameters(), lr=agent._sl_learning_rate)
        agent.policy_network_optimizer.load_state_dict(
            checkpoint['policy_network_optimizer'])
        agent._rl_agent.from_checkpoint(checkpoint['rl_agent'])
        agent._rl_agent.set_device(agent.device)
        return agent

    def save_checkpoint(self, path:str, filename:str='checkpoint_nfsp.pt')->None:
...
class ReservoirBuffer(object):
    ...
    @classmethod
    def from_checkpoint(cls, checkpoint:dict)->'ReservoirBuffer':
